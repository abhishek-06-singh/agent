...
const handleClick = (item) => {
  router.push(
    `/dashboard/agent/documents/${encodeURIComponent(item.name)}`
  )
}
              ...
              <tr key={item.id} className="hover:bg-boxbg transition">
                ...
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span onClick={() => handleClick(item)} className="text-sm text-textnormal font-medium cursor-pointer">{item.name}</span>
                  </div>
                </td>
                ...
                {/* Upload Date */}
                <td className="p-4 text-sm text-textnormal">{item.uploadDate}</td>
